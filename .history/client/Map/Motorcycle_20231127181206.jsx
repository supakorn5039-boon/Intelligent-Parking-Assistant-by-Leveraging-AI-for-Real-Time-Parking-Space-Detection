...
const Car = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const snapshot = await firestore().collection('available-lot').doc('your_document_id').get();
    ...
    <View style={styles.container}>
      ...
      <View>
        <TouchableOpacity onPress={handleButtonClick} style={styles.sq}>
          <Image style={styles.img} source={require('../assets/Photo/h2.jpeg')} />
          <View>
            <Text style={styles.com}>30th Computer</Text>
            <Text style={styles.des}>{`Available: ${parkingData.available || 0}`}</Text>
            <Text style={styles.timestamp}>{`Last Updated: ${parkingData.timestamp || 'N/A'}`}</Text>
          </View>
        </TouchableOpacity>
